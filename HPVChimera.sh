...
blast_85_div()
{
 ...
 ls|sort | while read filename;do
    ...
    blastn -db $hpv_database -query $filename -perc_identity 85  -outfmt "6 qseqid sseqid pident qcovs length mismatch gapopen qstart qend sstart send evalue bitscore" -out output/"$extension""_concoverageb.out"
    checkExit
  done 
}
